...
const Stocks = () => {
  ...
  const refreshData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/building_manager/stocks?paginate=1&page=${page}&perPage=${perPage}&sort=${sort}&order=${order}`);
    ...
    <>
      ...
      {selectedStock && (
        <>
          {/* <EditStockModal
            show={editStockModal}
            toggle={() => setEditStockModal(!editStockModal)}
            refreshData={refreshData}
            setLoading={setLoading}
            stock={selectedStock}
          /> */}
          <AddTransactionModal
            ...
            toggle={() => setAddTransactionModal(!addTransactionModal)}
            refreshData={refreshData}
            setLoading={setLoading}
            stock={selectedStock}
          />
      ...
      >
        <div className="pb-5 pt-2">
          <h3 className="text-center mb-1">انبارداری</h3>
          <p className="text-center mb-1">
            در این بخش می توانید موجودی انبار و کالاهای مجتمع را وارد نموده و به دلخواه خود به ساکنین نمایش دهید
          </p>
