...
export default ({ show, toggle, refreshData, setLoading, isModal = true }) => {
  ...
  return (
    ...
    >
      <ModalHeader toggle={toggle}>نوتیفیکیشن جدید</ModalHeader>
      <_AddFcmMessageModal
        show={show}
        toggle={toggle}
        setLoading={setLoading}
...
const _AddFcmMessageModal = ({ show, toggle, refreshData, setLoading }) => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const response = await axios.post("building_manager/fcmMessages", {
  ...
  const getUnits = async () => {
    try {
      const response = await axios.get("/building_manager/units?withResidents=1&onlyFcm=1");
  ...
  useEffect(() => {
    ...
    if (selectType == 'fcmmessage_mt') {
      setSelectedUnits(units.filter(unit => parseInt(unit.charge_fcmmessage) > parseInt(fcmmessage.replace(/,/g, ""))));
    }
    if (selectType == 'select') {
      setSelectedUnits([]);
    }
  }, [selectType, fcmmessage, units]);
  ...
  const variables = [
    ...
    {
      name: "مبلغ بدهی",
      value: "{fcmmessage_amount_in_rial}",
    },
  ];
  return (
    <ModalBody>
      <Form onSubmit={handleSubmit(onSubmit)}>
        <div className="mb-2">
          ...
          >
            ...
            <option value='fcmmessage_mt'>واحد ها با بدهی بیشتر از</option>
          </select>
        </div>
        {selectType == 'select' && (
          <div className="mb-2">
        ...
        {selectType == 'fcmmessage_mt' && (
          <div className="mb-2">
            <NumericFormat
              value={fcmmessage}
              onValueChange={(values) => {
                const { formattedValue, value } = values;
                setFcmMessage(formattedValue);
              }}
