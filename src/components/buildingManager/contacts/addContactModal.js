...
const AddContactModal = ({ show, toggle, refreshData, setLoading }) => {
  const {
    control,
    register,
    reset,
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const response = await axios.post("building_manager/contacts", data);
  ...
  return (
    <Modal isOpen={show} centered={true} size="lg" toggle={toggle}>
      <ModalHeader toggle={toggle}>افزودن مخاطب</ModalHeader>
      <ModalBody>
