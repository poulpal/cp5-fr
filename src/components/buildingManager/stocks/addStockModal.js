...
const AddStockModal = ({ show, setShow, setLoading, refreshData, isModal = true }) => {
  if (!isModal) {
    return (
  ...
  return (
    ...
    >
      <ModalHeader toggle={() => setShow(false)}>افزودن کالا</ModalHeader>
      <_AddStockModal
        show={show}
        setShow={setShow}
        setLoading={setLoading}
...
const _AddStockModal = ({ show, setShow, setLoading, refreshData }) => {
  ...
  const onSubmit = async (formData) => {
    setLoading(true);
    try {
      formData.price = formData.price.replace(/,/g, "") / (currency === "rial" ? 10 : 1);
      const response = await axios.post(
        "/building_manager/stocks",
        formData
      );
