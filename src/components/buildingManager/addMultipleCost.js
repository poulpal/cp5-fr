...
const AddMultipleCost = ({ units, setLoading, refreshData, toggleModal }) => {
  const [excelData, setExcelData] = useState([]);

  const handleSubmit = async () => {
    setLoading(true);
    const costs = excelData.map((item) => {
      return {
        amount: item["مبلغ"] / 10,
  ...
  const handleDownloadSample = () => {
    ...
    if (isNative() && window.ReactNativeWebView) {
      window.ReactNativeWebView.postMessage(JSON.stringify({
        ...
        type: "xlsx",
        name: "costs_sample" + fileExtension,
    ...
    FileSaver.saveAs(data, "costs_sample" + fileExtension);
  };
    ...
    <>
      ...
      {excelData.map((item, index) => (
        <div key={index}>
          ...
          <Row>
            <Col md="4">
              <Label for="monthly_charge">مبلغ هزینه</Label>
              <input
                name="monthly_charge"
                type="text"
...
export default AddMultipleCost;
